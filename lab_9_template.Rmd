...
homes_sub <- homes %>% 
  filter(City == "San Luis Obispo" |
         City == "Santa Maria-Orcutt" |
         City == "Atascadero" |
         City == "Arroyo Grande")

## what do you expect these relationships to look like? (e.g. what is the relationship between housing price and square footage? would you expect a home in foreclosure to sell for more or less than a regular house?)
...
by_sqft <- ggplot(homes_sub, aes(x = SqFt, y = Price)) +
  # pch is point style, depends on Status
...
# does it make sense to describe the relationship between sqft and price to be linear? yes - nothing that makes me think that it would be non-linear
# outlier in one regular Arroyo Grande house
# remember: the more variables, the harder it is for the audience to understand
...
# include all variables, but PricePerSqFt is going to bias the model because it is derived from the output variable you're trying to model (basically saying, look how related Price and PricePerSqFt are! surprise!!)
homes_lm1 <- lm(Price ~ City + Bedrooms + Bathrooms + SqFt + PricePerSqFt + Status, data = homes_sub)

# b. Model summary:
summary(homes_lm1)
# list of coefficients is the model equation, but this is a problem: If everything is held constant, if you increase the number of bedrooms by 1, the price of a house would decrease by $30 792. Since you know that this is impossible, the model probably doesn't work great. This, with other coefficients, show that the model is oversaturated and is probably biased by some colinearity.

# you can group variables: Bedrooms and Bathrooms and SqFt all basically describe size, so you can use SqFt and take Bedrooms and Bathrooms out
```
...
# b. Model summary:
summary(homes_lm2)
# just looking at coefficients: for every 1 sqft increase in a house, I expect the price of a house to increase by $245 on average. If you buy a house in a regular sale, it will be $209 998 more than a house bought in foreclosure.

# for short sale, p value indicates that the prices between Short Sale homes and Foreclosure homes are not significantly different.

# Arroyo Grande doesn't show up because it's the reference level (R chooses the first alphabetically). So I expect a house in Atascadero to sell for $202 157 less than a house in Arroyo Grande.

# some error is associated with this because you never get the exact model. 53% of variance in housing price is explained by this model (from adjusted R-squared). 

# Multiple R-squared doesn't adjust for the fact that as you increase the number of predictor variables, your model will go up. Adjusted R-squared adjusts for this, so it's always going to be a little lower. Both should be close.
```
...
# not actually overwriting anything, just saying "recognize this as a factor instead"
...
# use fct_relevel() in forcats
# puts regular first, then everything else after is alphabetical
# but you can do this in any order you want
# the first level becomes the reference level
homes_sub$Status <- fct_relevel(homes_sub$Status, "Regular")

# e. Now run the regression again - same equation, but now the reference level is different (Status = Regular): homes_sb
...
# a. Model diagnostics:
plot(homes_lm3)
# Residuals vs Fitted: for all observations in dataset, no clear trend in terms of variance (spread is pretty even), looks heteroscedastic. 188 sold for less than what we would expect it to
# normal Q-Q: residuals are normally distributed (with 3 outlier residuals)
```
...
# If you were using AIC only, you would pick homes_lm1 (because AIC = 10699.25) over homes_lm3 (AIC = 11148). However, homes_lm1 includes a variable that is directly derived from output variable, so this model isn't justified based on your own conceptual understanding of the data.
```
...
# output is scary but shows up well in html
...
# then you get a vector of numeric values, but you need to bind to the data to make it useful

# b. Bind predictions to the data to make it actually useful:
predict_df <- data.frame(df_new, price_predict)

# fit.fit is prediction for house price
```
...
predict_graph <- ggplot(predict_df, aes(x = SqFt, y = fit.fit)) +
  ...
  geom_point(data = homes_sub, aes(x = SqFt, y = Price), 
             alpha = 0.5) +
  facet_wrap(~ Status)
# you can add original data onto an existing ggplot graph as long as you call the dataset you want to plot from
predict_graph
